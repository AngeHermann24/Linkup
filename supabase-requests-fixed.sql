-- Script SQL pour le système de demandes de services (version corrigée)
...
-- 4. Fonction pour mettre à jour updated_at (si elle n'existe pas déjà)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'update_updated_at_requests') THEN
    CREATE FUNCTION update_updated_at_requests()
...
-- 5. Trigger pour updated_at (supprimer s'il existe, puis recréer)
